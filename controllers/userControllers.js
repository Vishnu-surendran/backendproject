  ...
  try {
    ...
    async function main() {
      ...
      let info = await transporter.sendMail({
        ...
        text: `Your link for resetting password has been given below.https://peppy-unicorn-0ac1b9.netlify.app/resetpassword/${token}`,
      });
    }
    main();
...
try{
  const worker=await Booking.find({customerid:id}).sort({_id:-1})
